...
final class CRMSpeedPhoneDashlet extends Dashlet
{
    ...
    public function display()
    {
        ...
        return parent::display()
            ...
            . '<h3 data-speedphone-incoming-title style="margin:4px 32px 4px 0;">Kontakt in SpeedPhone öffnen</h3>'
            ...
            . 'const opt=JSON.parse(box.dataset.options);const number=box.querySelector("[data-speedphone-incoming-number]");const heading=box.querySelector("[data-speedphone-incoming-title]");'
            ...
            . 'function show(call){if(!call||!call.event_id){box.hidden=true;current="";return;}if(current===call.event_id&&!box.hidden)return;current=call.event_id;number.textContent="Anrufer: "+(call.caller_phone||"Nummer nicht übermittelt");matches.replaceChildren();const found=Array.isArray(call.matches)&&call.matches.length>0;heading.textContent=found?"Kontakt in SpeedPhone öffnen":"Nummer nicht im CRM gefunden";if(!found){const hint=document.createElement("p");hint.style.cssText="margin:4px 0;color:#617080";hint.textContent="Der Anruf wurde erkannt, aber diese Rufnummer ist bei keinem freigegebenen CRM-Kontakt hinterlegt.";matches.append(hint);}(call.matches||[]).forEach(match=>{const button=document.createElement("button");button.type="button";button.style.cssText="padding:11px 12px;border:1px solid #d9e1e8;border-radius:10px;background:#fff;text-align:left;cursor:pointer";const contactTitle=document.createElement("strong");contactTitle.textContent=match.display_name||"Kontakt ohne Namen";const detail=document.createElement("small");detail.style.cssText="display:block;margin-top:3px;color:#155e75";detail.textContent=[match.city,match.match_label].filter(Boolean).join(" · ");button.append(contactTitle,detail);button.addEventListener("click",async()=>{button.disabled=true;try{await send("open_incoming_pbx",{event_id:call.event_id,prospect_id:match.prospect_id});window.top.location.href=opt.speedphone;}catch(error){button.disabled=false;window.alert(error.message);}});matches.append(button);});box.hidden=false;}'
